...
import { MapPin, Star, Heart, ChevronRight, Home as HomeIcon, ReceiptText, Store as StoreIcon, SlidersHorizontal, Clock3, RotateCcw, Bookmark, Ticket, Crown, UtensilsCrossed, Soup, Coffee, Cookie, Beef, Sandwich, Check } from 'lucide-react'
  ...
  return (
    <div className="min-h-dvh">
      <main className={`mx-auto max-w-[480px] ${view.page==='store'?'pb-24':'pb-[64px]'}`}>
        ...
        {view.page==='home' && tab==='home' && <Home stores={stores} loading={loading} loadErr={loadErr} onRetry={loadStores} onOpen={id=>setView({page:'store',id})} onGoTrack={()=>setTab('track')} />}
...
function Home({stores,loading,loadErr,onRetry,onOpen,onGoTrack}:{stores:StoreType[];loading:boolean;loadErr:string|null;onRetry:()=>void;onOpen:(id:string)=>void;onGoTrack?:()=>void}){
  ...
  return (
    <div className="bg-[var(--color-bg)]">
      ...
      <header className="sticky top-0 z-20 border-b border-[var(--color-border)] bg-white/90 px-4 pb-3 pt-3 backdrop-blur supports-[backdrop-filter]:bg-white/80">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button aria-label={`ตะกร้า ${cartCount} รายการ`} onClick={()=>onGoTrack?.()} className="pressable relative flex h-10 w-10 items-center justify-center rounded-full border border-[var(--color-border)] bg-white transition active:scale-[0.97]">
      ...
      <section className="bg-white px-4 pb-3">
        <button onClick={()=>setShowCoupon(true)} className="pressable flex w-full items-center gap-4 rounded-[16px] border border-[var(--color-border)] bg-[var(--color-accent-soft)] p-4 text-left transition active:scale-[0.97]">
      ...
      <section className="bg-white px-4 pb-3 pt-3">
        ...
        <div className="no-scrollbar flex gap-2 overflow-x-auto snap-x snap-mandatory pb-1" style={{scrollbarWidth:'none'}}>
          {CAT_DEFS.map(c=>(
            <Chip key={c.id} label={c.label} Icon={c.Icon} active={cat===c.id} onClick={()=>setCat(c.id)} />
          ))}
        </div>
      </section>
